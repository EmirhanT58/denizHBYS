...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["hasta_ekle"])) {
    ...
    try {
        $stmt = $db->prepare("INSERT INTO hastalar (tc, ad, soyad, cinsiyet, dogum_tarihi, telefon, email, adres, kan_grubu) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$tc, $ad, $soyad, $cinsiyet, $dogum_tarihi, $telefon, $email, $adres, $kan_grubu]);
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["hasta_guncelle"])) {
    ...
    try {
        ...
        $stmt = $db->prepare("UPDATE hastalar SET 
    tc = ?, 
    ad = ?, 
    soyad = ?, 
    cinsiyet = ?, 
    ...
    $stmt->execute([$g_tc, $g_ad, $g_soyad, $g_cinsiyet, $g_dogum_tarihi, $g_telefon, $g_email, $g_adres, $g_kan_grubu, $g_id]);
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            ...
            <button onclick="document.getElementById('hastaModal').classList.remove('hidden')"
                class="bg-primary text-white py-2 px-4 rounded-lg">Yeni Hasta Ekle</button>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bilgiler
                    </th>
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-gray-500">TCKN: <?php echo htmlspecialchars($hasta['tc']); ?>
                                </div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900"><?php echo htmlspecialchars($hasta['cinsiyet']); ?></div>
                        <div class="text-sm text-gray-500">
                            <?php echo date('d/m/Y', strtotime($hasta['dogum_tarihi'])); ?>
                        </div>
                        <div class="text-sm text-gray-500"><?php echo htmlspecialchars($hasta['kan_grubu']); ?></div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <div class="text-xs text-gray-400 truncate max-w-xs">
                            <?php echo htmlspecialchars($hasta['adres']); ?>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a onclick="hastaDuzenleModal( '<?php echo $hasta['id']; ?>',
                          ...
                          '<?php echo htmlspecialchars($hasta['adres'], ENT_QUOTES); ?>',
                          '<?php echo htmlspecialchars($hasta['kan_grubu'], ENT_QUOTES); ?>')"
   
                            class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-edit"></i></a>
                        <a href="dashboard.php?sayfa=hastalar&sil=<?php echo $hasta['id']; ?>"
...
<!-- Hasta Ekle Modal -->
<div id="hastaModal" class="fixed inset-0 z-50 hidden overflow-y-auto m-auto mt-[190px]">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Hasta Bilgisi Ekle</h3>

                <form method="POST" id="hasta_guncelle">
                    ...
                    <div class="grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6">
                        ...
                        <div class="sm:col-span-3">
                            <label for="guncelle" class="block text-sm font-medium text-gray-700">Cinsiyet</label>
                        ...
                        <div class="sm:col-span-3">
                            <label for="dogum_tarihi" class="block text-sm font-medium text-gray-700">Doğum
                                Tarihi</label>
                    ...
                    <div class="mt-5 sm:mt-6 flex justify-end space-x-3">
                        <button type="button"
                            onclick="document.getElementById('hastaGuncelleModal').classList.add('hidden')"
...
<div id="hastaGuncelleModal" class="fixed inset-0 z-50 hidden overflow-y-auto m-auto mt-[190px]">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 transition-opacity" aria-hidden="true">
            <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
        </div>
        ...
        <div
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form method="POST" id="guncelleForm">
                    ...
                    <div class="grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6">
                        <div class="sm:col-span-2">
                            <label for="g_tc" class="block text-sm font-medium text-gray-700">TC Kimlik
                                No</label>
                            <input type="text" name="g_tc" id="guncelle_tc" required
                                class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2">
                        </div>
                        ...
                        <div class="sm:col-span-3">
                            <label for="guncelle_cinsiyet"
                                class="block text-sm font-medium text-gray-700">Cinsiyet</label>
                        ...
                        <div class="sm:col-span-3">
                            <label for="guncelle_dogum_tarihi" class="block text-sm font-medium text-gray-700">Doğum
                                Tarihi</label>
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="g_telefon" id="guncelle_telefon" required
                                class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2">
                        </div>
                        ...
                        <div class="sm:col-span-3">
                            <label for="guncelle_kan_grubu" class="block text-sm font-medium text-gray-700">Kan
                                Grubu</label>
